...
#### Important points
- Given a sentence (string), return a new string with the order of words reversed but keep characters in each word same
- Words may be separated by multiple spaces; output should have single spaces only
- Leading and trailing spaces should be trimmed

#### Solution program
```js
// Approach: split on whitespace, filter empty strings, reverse array, join with single space
...
console.log(reverseWords("  one two   three  ")) // Output: three two one
